 ...
 *
 * @param X Offset
 * @desc The number of pixels to offset hint horizontally
 * fade the hint.
 * @default 0
 *
 * @param Y Offset
 * @desc The number of pixels to offset hint vertically
 * fade the hint.
 * @default 0
 *
 *
...
Khas.Hint.XOFF = Number(Khas.Hint.PARAMETERS["X Offset"]);
Khas.Hint.YOFF = Number(Khas.Hint.PARAMETERS["Y Offset"]);
...
function Khas_Hint() { this.initialize.apply(this, arguments); };
  ...
  Khas_Hint.prototype.setPos = function(x, y) {
    this._sprite.x = x - Math.round(this._width * 0.5) + Khas.Hint.XOFF;
    this._sprite.y = y - Math.round(this._height * 0.5) + Khas.Hint.YOFF;
  };
