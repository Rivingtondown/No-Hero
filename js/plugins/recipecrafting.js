 ...
 * @desc Crafting Exp gained text. (default - Crafting Exp Gained:)
 ...
 *  $gameParty.gainCraftingExp(craftingName, value)   (Adds exp to a specific category)
 *
 * Recipe Setup
...
(function() {
	...
	var tempdParam = "gold:true, chance:true, plevel:true, clevel:true, exp:true"
	...
	var expCost = parameters['Exp Gained Text'] || 'Crafting Exp Gained:';
	...
	Game_Party.prototype.getCraftLevel = function(string) {
		return this.craftLevel(this.getCategoryIndex(string));
	}
	Game_Party.prototype.getCategoryIndex = function(string) {
		var index = categoryNames.indexOf(string)
		if(index >= 0) {
			return categoryNames.indexOf(string);
		} else {
	...
	Game_Party.prototype.gainCraftingExp = function(string, value) {
		this.gainCraftExp(this.getCategoryIndex(string), value);
	}
	...
	Window_RecipeDetail.prototype.initialize = function(x, y, w, h, confirmWindow) {
	...
	Window_RecipeDetail.prototype.update = function() {
		Window_Base.prototype.update.call(this);
		if(this._confirmWindow.amountChanged()) {
			this.refresh();
		}
	}
	...
	Window_RecipeDetail.prototype.contentsHeight = function() {
		...
		if(this._recipe) {
			...
			if(displayParam.exp) { height += this.lineHeight(); }
	...
	Window_RecipeDetail.prototype.refresh = function() {
		...
		if(this._recipe) {
			if(!this._reverse) {
				...
				if(displayParam.gold) {this.drawGoldCost(bottomY);bottomY -= this.lineHeight(); }
				if(displayParam.exp) {this.drawExpGained(bottomY);}
			}
			this.drawMaterials();
		}
	}
	...
	Window_RecipeDetail.prototype.drawMaterials = function() {
		...
		for(var i = 0;i < this._recipe._materials.length;i++) {
			...
			if(this._reverse) {
				...
				this.changeTextColor(this.normalColor());
				string = " / " + String(this.getItemAmount(item._amount));
			}
			this.drawText(string,0,yy,this.contents.width-12,"right");
			yy += this.contents.fontSize  + this.textPadding();
		}
	}
	Window_RecipeDetail.prototype.getItemAmount = function(value) {
		return value * this._confirmWindow._amount;
	}
	...
	Window_RecipeDetail.prototype.drawGoldCost = function(yy) {
		if(this.getRecipeGoldCost() > 0) {
			this.changePaintOpacity(this._recipe.hasGold());
			this.changeTextColor(this.systemColor());
			this.drawText(craCost,0,yy,this.contents.width);
			this.changeTextColor(this.normalColor());
			this.drawCurrencyValue(this.getRecipeGoldCost(),TextManager.currencyUnit,0,yy,this.contents.width)
		}
	}
	Window_RecipeDetail.prototype.drawExpGained = function(yy) {
		if(this.getRecipeXpGain() > 0) {
			this.changeTextColor(this.systemColor());
			this.drawText(expCost, 0, yy, this.contents.width);
			this.changeTextColor(this.normalColor());
			this.drawText(this.getRecipeXpGain(), 0, yy, this.contents.width,"right");
		}
	}
	...
	Window_RecipeDetail.prototype.getRecipeXpGain = function() {
		return this._recipe.xpGain() * this._confirmWindow._amount;
	}
	...
	Scene_Crafting.prototype.create = function() {
		...
		this._confirmWindow = new Window_RecipeConfirm(width,0,width,72);
		...
		this._detailWindow = new Window_RecipeDetail(width,this._listWindow.y,width,height-72*2,this._confirmWindow);
		this._detailWindow.createContents();
		height = this._detailWindow.y + this._detailWindow.height;
		this._confirmWindow.y = height;
