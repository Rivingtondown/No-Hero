...
(function() {
	...
	var mogtint = [ '#555555', '#FFEFD5', '#FFFFFF', '#EEE8AA', '#555555', '#111111' ];
	...
	Lightmask.prototype._updateMask = function() {
		...
		if (scriptactive == true) {
		    ...
		    this._maskBitmap.fillRect(0, 0, maxX, maxY, '#111111');
			...
			if (player_radius > 0) {
				if (playerflashlight == true) {
					this._maskBitmap.radialgradientFillRect2(x1,y1, 20, player_radius, playercolor, '#111111', pd, flashlightlength, flashlightwidth);
				}
				if (player_radius < 100){
					...
					this._maskBitmap.radialgradientFillRect(x1,y1, 0, player_radius, newcolor, 'transparent', playerflicker,playerbrightness);
				} else {
					this._maskBitmap.radialgradientFillRect(x1,y1, 20, player_radius, playercolor, 'transparent', playerflicker,playerbrightness);
				}
		            ...
		            if (note_command == "light" || note_command == "fire" || note_command == "flashlight") {
			        	...
			        	if (light_radius >= 0) {
				            ...
				            if (state == true) {
								...
								if (flashlight == true) {
									this._maskBitmap.radialgradientFillRect2(lx1,ly1, 0, light_radius , colorvalue, 'transparent', ldir, flashlength, flashwidth);
								} else {
					            	this._maskBitmap.radialgradientFillRect(lx1,ly1, 0, light_radius , colorvalue, 'transparent', objectflicker, brightness, direction);
								}
					        }
		            	}
		            }
							...
							for (var x = 0; x < $dataMap.width; x++) {
								...
								if (tag == tile_number) {
									...
									if (tile_type == 3 || tile_type == 4) {
										this._maskBitmap.radialgradientFillRect(x1,y1, 0, tile_radius , tile_color, 'transparent', false, brightness); // Light
									} else {

										this._maskBitmap.radialgradientFillRect(x1,y1, 0, tile_radius , tile_color, 'transparent', true, brightness);  // Fire
									}
					    		}
				        	}
			        	}
