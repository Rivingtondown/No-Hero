...
(function()
{
	var Controls = EvilCat.Controls = function Controls()
	{
		EvilCat.Plugin.call(this);
	}
	EvilCat.extend(Controls, EvilCat.Plugin);
	Controls = EvilCat.Controls = new Controls();
	
	EvilCat.parsers.set('Key', function(param)
	{
		if (param instanceof Key) return param;
		return Key.parse(param);
	});
	
	EvilCat.parsers.set('Keys', function(param)
	{
		if (param instanceof Keys) return param;
		return Keys.parse(param);
	});
	
	var Key = EvilCat.Key = function(code)
	{
		this.code=code;
	}
		
	Key.prototype.recognize=function(event) // recognizes the key in JavaScript event
	{
		return false;
		// overload me!
	}
		
	Key.parse=function(code)
	{
	...
	var KeyCode = EvilCat.KeyCode = function(code)
	{
		Key.call(this, code);
	}
	EvilCat.extend(KeyCode, Key);
	
	KeyCode.prototype.recognize=function(event)
	{
		return event.keyCode && event.keyCode===this.code;
	}
		
	KeyCode.parse=function(code)
	{
	...
	var Keys = EvilCat.Keys = function(keys)
	{
		this.keys=Keys.keys_from_codes(keys);
	}
		
	Keys.prototype.recognize=function(event)
	{
		return this.keys.some(function(key) { return key.recognize(event); });
	}
		
	Keys.parse=function(code)
	{
	...
	Keys.keys_from_codes=function(codes)
	{
